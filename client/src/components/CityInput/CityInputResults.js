...
import { citiesReset, fetchCities, setInputValue, addPreviousCities, setCityToSearch } from '../../redux';
import CityList from './CityList';

function CityInputResults() {
  const dispatch = useDispatch();

  const handleCityClick = (id, name, lon, lat, country) => {
    dispatch(setInputValue(name));
    dispatch(addPreviousCities({ id, name, lon, lat, country }));
...
export default CityInputResults;
